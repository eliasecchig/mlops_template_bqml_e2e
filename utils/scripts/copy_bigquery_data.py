...
def get_batch_data_bq(PROJECT):
    ...
    |-`txlabels` (table: transactions with transactions labels (1 or 0))
    |-demographics
...
def create_bucket_if_not_exists(bucket_name: str, location: str):
...
def parse_args():
    ...
    parser.add_argument("--location", help="Gcp location for bucket and bq dataset", default='us-central1')
...
if __name__ == "__main__":
    args = parse_args()
    bucket_name = f"{args.project}-dummy-data"

    create_bucket_if_not_exists(bucket_name=bucket_name, location=args.location)
    get_batch_data_gcs(bucket_name)
    get_batch_data_bq(args.project)